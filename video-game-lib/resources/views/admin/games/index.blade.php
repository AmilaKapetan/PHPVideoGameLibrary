...
<div class="container text-center text-white">
    ...
    <div>
        <a class="btn btn-outline-light col" href="{{ route('admin.showCategories') }}">Manage Categories</a>
        <a class="btn btn-outline-light col"  href="{{ route('admin.addGame') }}">Add New Game</a>
    </div>
    ...
    @foreach ($games as $game)
        <div class="row mt-2 mb-2">
            ...
            <button class="col btn btn-outline-light" data-bs-toggle="modal" data-bs-target="#editModal" data-bs-gameid="{{ $game->id }}">Edit</button>
            <form class="col" action="{{ route('admin.deleteGame', $game->id) }}" method="post">
                @csrf
                <button class="col btn btn-outline-light" type="submit">Delete</button>
            </form>
        </div>
        <hr class="text-white mx-3 my-4">
    @endforeach
